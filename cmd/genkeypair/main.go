...
func main() {
	...
	if len(cfg.PrivateKey) > 0 {
		...
		PrivateKey, err := secp256k1.DeserializeSchnorrPrivateKeyFromSlice(pk)
		if err != nil {
			panic(err)
		}
		...
		Address, err := util.NewAddressPublicKey(SerializedPublicKey[:], cfg.NetParams().Prefix)
		if err != nil {
			panic(err)
		}

		fmt.Printf("Private key: %s\n", PrivateKey.SerializePrivateKey().String())
